/* DTrackSDK: C++ source file, A.R.T. GmbH 21.4.05-25.1.07
 *
 ...
 *
 * $Id: DTrack.cpp,v 1.5 2007/02/19 17:54:43 kurt Exp $
 */
        ...
        )
{
    ...
    if(remote_host != NULL && remote_port != 0){
        if(!(d_remote_ip = ip_name2ip(remote_host))){
            d_remote_port = 0;
        }else{
        ...
        if(!d_remote_ip){  // resolving of hostname was not possible
            free(d_udpbuf);
            udp_exit(d_udpsock);
            d_udpsock = NULL;
            return;
